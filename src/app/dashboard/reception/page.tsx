...
export default function ReceptionDashboard() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        // Construct the WhatsApp Message based on visit type
        ...
        try {
          fetch('/api/whatsapp', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ phone: formData.phone, message: realWaMessage, templateId: formData.visitDate ? 'FUTURE_BOOKING' : 'TODAY_VISIT' })
          });
